classdef DbSlice2
    ...
    % [nndb_tr, ~, ~, ~] = DbSlice2.slice(nndb, sel);
    ...
    methods (Access = public, Static) 
        ...
        function [nndbs_tr, nndbs_val, nndbs_te, nndbs_tr_out, nndbs_val_out] = slice(nndb, sel) 
            ...
            type         = class(nndb.db);  
            nndbs_tr     = DbSlice2.init_nndb('Training', type, sel, nndb, tr_n_per_class, numel(cls_range), true);
            nndbs_tr_out = DbSlice2.init_nndb('Cannonical', type, sel, nndb, tr_out_n_per_class, numel(cls_range), false);
            nndbs_val    = DbSlice2.init_nndb('Validation', type, sel, nndb, val_n_per_class, numel(val_cls_range), true);   
            nndbs_val_out= DbSlice2.init_nndb('ValCannonical', type, sel, nndb, val_out_n_per_class, numel(val_cls_range), false);
            nndbs_te     = DbSlice2.init_nndb('Testing', type, sel, nndb, te_n_per_class, numel(te_cls_range), true);
                       
            % Fetch iterative range
            data_range = DbSlice2.get_data_range(cls_range, val_cls_range, te_cls_range, ...
            ...
            for i=data_range
                ...
                p = ~DbSlice2.dicard_needed(nndbs_tr, i, i_cls, cls_range, prev_cls_en, sel.tr_col_indices);
                p = p | ~DbSlice2.dicard_needed(nndbs_tr_out, i, i_cls, cls_range, prev_cls_en, sel.tr_out_col_indices);
                p = p | ~DbSlice2.dicard_needed(nndbs_val, i, i_cls, val_cls_range, prev_cls_en, sel.val_col_indices);
                p = p | ~DbSlice2.dicard_needed(nndbs_val_out, i, i_cls, val_cls_range, prev_cls_en, sel.val_out_col_indices);
                p = p | ~DbSlice2.dicard_needed(nndbs_te, i, i_cls, te_cls_range, prev_cls_en, sel.te_col_indices);
                if (~p); continue; end
                ...
                for pI=1:patch_loop_max_n 
                    ...
                    if (nndb.format == Format.H_W_CH_N)
                        ...
                        img = DbSlice2.process_color(img, sel);
                        cls_st_img = DbSlice2.process_color(cls_st_img, sel);
                    ...
                    % Build Training DB
                    [nndbs_tr, tr_idxs] = ...
                        DbSlice2.build_nndb_tr(nndbs_tr, pI, tr_idxs, i, i_cls, cls_range, prev_cls_en, img, sel, nf);

                    % Build Training Target DB
                    [nndbs_tr_out, tr_out_idxs] = ...
                        DbSlice2.build_nndb_tr_out(nndbs_tr_out, pI, tr_out_idxs, i, i_cls, cls_range, prev_cls_en, img, sel);

                    % Build Valdiation DB
                    [nndbs_val, val_idxs] = ...
                        DbSlice2.build_nndb_val(nndbs_val, pI, val_idxs, i, i_cls, val_cls_range, prev_cls_en, img, sel);

                    % Build Valdiation Target DB
                    [nndbs_val_out, val_out_idxs] = ...
                        DbSlice2.build_nndb_val_out(nndbs_val_out, pI, val_out_idxs, i, i_cls, val_cls_range, prev_cls_en, img, sel);
                    
                    % Build Testing DB
                    [nndbs_te, te_idxs] = ...
                        DbSlice2.build_nndb_te(nndbs_te, pI, te_idxs, i, i_cls, te_cls_range, prev_cls_en, img, sel);
                end
            end              
        ...
        function examples(imdb_8)
            ...
            % sel.pre_process_script    = [];
            % [nndb_tr, ~, nndb_te, ~] = DbSlice2.slice(nndb, sel); 
            ...
            [nndb_tr, ~, ~, ~] = DbSlice2.slice(nndb, sel); % nndb_tr = DbSlice2.slice(nndb, sel); 
            ...
            [nndbs_tr, ~, ~, ~] = DbSlice2.slice(nndb, sel);
            nndbs_tr{1}.show()
            figure, 
            nndbs_tr{2}.show()
            figure, 
            ...
            sel.te_col_indices = [3 5];     %[3 5]; 
            sel.class_range    = [1:10];    % First ten identities 
            [nndb_tr, nndb_val, nndb_te, ~] = DbSlice2.slice(nndb, sel);
            ...
            sel.val_class_range     = [6:15];
            sel.te_class_range      = [17:20];
            [nndb_tr, nndb_val, nndb_te, nndb_tr_out, nndb_val_out] = DbSlice2.slice(nndb, sel);
            nndb_tr.show(10, 3)
            ...
            sel.te_col_indices = [3 5];         %[3 5]; 
            [nndb_tr, nndb_tr_out, nndb_te, ~] = DbSlice2.slice(nndb, sel);
	...
	methods (Access = private, Static) 
        ...
        function [new_nndbs] = init_nndb(name, type, sel, nndb, n_per_class, cls_n, build_cls_idx) 
            ...
            for i=1:numel(db)
                new_nndbs{i} = NNdb(name, db{i}, n_per_class, build_cls_idx, [], nndb.format);  
            end
        end
        ...
        function [nndbs, ni] = build_nndb_tr(nndbs, pi, ni, i, i_cls, cls_range, prev_cls_en, img, sel, noise_found) 
            ...
            [discard, found] = DbSlice2.dicard_needed(nndbs, i, i_cls, cls_range, prev_cls_en, sel.tr_col_indices);               
...
%                             img = imnoise(img, 'gaussian');
                        ...
                        % Set the corruption
                        cimg = uint8(DbSlice2.rand_corrupt(cs(1), cs(2)));
...
%             rand('twister',100*sum(clock));
        ...
        function [nndbs, ni] = build_nndb_tr_out(nndbs, pi, ni, i, i_cls, cls_range, prev_cls_en, img, sel) 
            ...
            % Find whether 'i' is in required indices                
            [discard, found] = DbSlice2.dicard_needed(nndbs, i, i_cls, cls_range, prev_cls_en, sel.tr_out_col_indices);                
            if (discard); return; end; 
        ...
        function [nndbs, ni] = build_nndb_val(nndbs, pi, ni, i, i_cls, cls_range, prev_cls_en, img, sel) 
            ...
            % Find whether 'i' is in required indices                
            [discard, found] = DbSlice2.dicard_needed(nndbs, i, i_cls, cls_range, prev_cls_en, sel.val_col_indices);                
            if (discard); return; end; 
        ...
        function [nndbs, ni] = build_nndb_val_out(nndbs, pi, ni, i, i_cls, cls_range, prev_cls_en, img, sel) 
            ...
            % Find whether 'i' is in required indices                
            [discard, found] = DbSlice2.dicard_needed(nndbs, i, i_cls, cls_range, prev_cls_en, sel.val_out_col_indices);                
            if (discard); return; end; 
        ...
        function [nndbs, ni] = build_nndb_te(nndbs, pi, ni, i, i_cls, cls_range, prev_cls_en, img, sel) 
            ...
            % Find whether 'i' is in required indices                
            [discard, found] = DbSlice2.dicard_needed(nndbs, i, i_cls, cls_range, prev_cls_en, sel.te_col_indices);                
            if (discard); return; end;  
        ...
        function [discard, found] = dicard_needed(nndbs, i, i_cls, cls_range, prev_cls_en, col_indices) 
            ...
            found = DbSlice2.find(i, prev_cls_en, col_indices);            
