classdef KSVD
    %KSVD: k-mean SVD algorithm and varients.
    ...
    % Copyright 2015-2016 Slinda, Curtin University (slinda1983@gmail.com).
    
    methods (Access = public, Static)
    	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Public Interface
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
		function [D,X] = l1(nndb, info) 
            % L1: leanrs the KSVD subspace with l1-norm     
            %
            % Parameters
            % ----------
            % nndb : nnf.db.NNdb
            ...
            %     Provide additional information to perform KSVD. (Default value = []).    
            %        
            ...
            %
            %     info.errorFlag = 0;       % if =0, a fix number of coefficients is
            ...
            %                                 error.
            % 
            %     info.preserveDCAtom = 0;  % if =1 then the first atom in the dictionary
            ...
            %                                 atoms are trained).
            %
            %     info.displayProgress = 1;  % if =1 progress information is displyed. If param.errorFlag==0, 
            ...
            %
            %     info.InitializationMethod = 'DataElements';   % (optional) mehtod to initialize the dictionary, can
            ...
            if (~isfield(info,'L')); info.L = 3; end; 
            if (~isfield(info,'K')); info.K = nndb.cls_n; end;  
            if (~isfield(info,'numIteration')); info.numIteration = 50; end;  
            if (~isfield(info,'errorFlag')); info.errorFlag = 0; end; 
            if (~isfield(info,'errorGoal')); info.errorGoal = 1e-15; end;
            if (~isfield(info,'preserveDCAtom')); info.preserveDCAtom = 0; end; 
            if (~isfield(info,'InitializationMethod')); info.InitializationMethod = 'DataElements'; end; 
            if (~isfield(info,'displayProgress')); info.displayProgress = 1; end; 
                        
            % Perform KSVD
            [D,X] = KSVD.optimize(nndb.features,info);
        end
    ...
    methods (Access = private, Static)
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function [Dictionary,output] = optimize(...
            ...
            for iterNum = 1:param.numIteration
                ...
                for j = rPerm
                    [betterDictionaryElement,CoefMatrix,addedNewVector] = KSVD.I_findBetterDictionaryElement(Data,...
                ...
                if (displayErrorWithTrueDictionary ) 
                    [ratio(iterNum+1),ErrorBetweenDictionaries(iterNum+1)] = KSVD.I_findDistanseBetweenDictionaries(param.TrueDictionary,Dictionary);
                    disp(strcat(['Iteration  ', num2str(iterNum),' ratio of restored elements: ',num2str(ratio(iterNum+1))]));
                    output.ratio = ratio;
                end
                Dictionary = KSVD.I_clearDictionary(Dictionary,CoefMatrix(size(FixedDictionaryElement,2)+1:end,:),Data);
